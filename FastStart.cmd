...
python --version
if errorlevel 1 (
    echo 错误: 未检测到 Python，请确保 Python 3.8+ 已安装
    pause
    exit /b 1
)
echo.
...
REM 检查依赖
echo 检查 Python 依赖...
if exist "deepdiver_v2\requirements.txt" (
...
REM 检查配置文件
echo 检查配置文件...
if exist "deepdiver_v2\config\.env" (
    echo [OK] 环境变量配置文件存在: deepdiver_v2\config\.env
    echo   请编辑 deepdiver_v2\config\.env 配置你的 API 密钥和数据库信息
) else (
    echo [X] 环境变量配置文件不存在
    echo   请运行: copy deepdiver_v2\config\env.template deepdiver_v2\config\.env
    echo   然后编辑 deepdiver_v2\config\.env 配置你的 API 密钥和数据库信息
)

if exist "deepdiver_v2\src\tools\server_config.yaml" (
    echo [OK] MCP 服务器配置文件存在
) else (
    echo [X] MCP 服务器配置文件不存在
)
echo.

REM 检查数据库
echo 检查数据库配置...
if exist "chatAi\chatai.sql" (
    echo [OK] 数据库 SQL 文件存在: chatAi\chatai.sql
    echo   如需初始化数据库，请运行:
    echo   mysql -u root -p ^< chatAi\chatai.sql
) else (
    echo [X] 数据库 SQL 文件不存在
)
echo.
...
echo ==========================================
echo.
echo 需要启动 3 个服务，请在不同的命令行窗口中运行以下命令:
echo.
echo 【服务 1】MCP 服务器（搜索服务）
echo   cd deepdiver_v2
echo   python src\tools\mcp_server_standard.py --config src\tools\server_config.yaml
echo.
